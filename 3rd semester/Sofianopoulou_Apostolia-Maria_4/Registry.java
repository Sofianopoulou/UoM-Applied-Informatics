...
public class Registry {
	...
	public void addCommunication(Communication aCommunication) {
		...
		{
			if(AllSuspects.get(i).getNumbers().contains(aCommunication.num1)) 
			{
				for(int j=0;j<AllSuspects.size();j++)
				{
					if(AllSuspects.get(j).getNumbers().contains(aCommunication.num2))
					{
						AllSuspects.get(j).getPossPartners().add(AllSuspects.get(i)); 
						AllSuspects.get(i).getPossPartners().add(AllSuspects.get(j));
					}
				}
			}
		}
	...
	public Suspect getSuspectWithMostPartners() {
		...
		for(int i=0;i<AllSuspects.size();i++)
			for(int j=0;j<AllSuspects.get(i).getPossPartners().size();j++) 
				{
					if(AllSuspects.get(i).getPossPartners().size() > max)
					{
						max = AllSuspects.get(i).getPossPartners().size();
						index=i;
					}
				}
